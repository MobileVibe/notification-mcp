...
if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID) {
    throw new Error("Missing required environment variables. Please ensure TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID are set.");
}
                ...
                while (Date.now() - startTime < timeoutMs) {
                    ...
                    if (updatesResponse.data.ok) {
                        ...
                        const newMessages = updatesResponse.data.result
                            .filter((update) => update.message && update.message.message_id > response.data.result.message_id)
            ...
            try {
                ...
                if (debugResponse.data.ok) {
                    ...
                    const recentMessages = debugResponse.data.result
                        .filter((update) => update.message && update.message.message_id > messageId)
                        .sort((a, b) => a.message.message_id - b.message.message_id);
